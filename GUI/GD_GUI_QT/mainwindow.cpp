    ...
    ui(new Ui::MainWindow)
{
    ...
    CSwitchWidget* wigt3 = new CSwitchWidget(this);
    wigt3->SetPressStyle(GRAPH_PRESSSTYLE_SYSTEM2);
    wigt3->setGeometry(QRect(50, 630, 150, 50));
    wigt3->SetSwitchState(1);
    ...
    connect(wigt3,SIGNAL(StateChanged(int)),this,SLOT(switch3sataechanged(int)));
...
//按钮状态改变
void MainWindow::switch3sataechanged(int i)
{
    qDebug() << i;
    if(i == 1)
    {
        QMQTT::Message mqtt_message(0,"/room/led1/ON","ON",0);
        client->publish(mqtt_message);
    }
    else if(i == 0)
    {
        QMQTT::Message mqtt_message(0,"/room/led1/OFF","OFF",0);
        client->publish(mqtt_message);
    }
